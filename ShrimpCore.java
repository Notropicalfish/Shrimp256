public class ShrimpCore{
  ...
  private static byte[] runEncryptionRound(byte[] block, String key, int roundNumber){
    ...
    for (int i = 0; i < block.length; i++){
      ...
      int scrambled = (block[i] ^ keyBytes[keyIndex]) + roundNumber;
      block[i] = rotateLeft((byte)scrambled, (i + roundNumber) % 8);
    }
    return block;
  }

  public static byte[] decryptBlock(byte[] data, String key, int rounds){
    byte[] block = data.clone();
  ...
  private static byte[] runDecryptionRound(byte[] block, String key, int roundNumber){
    ...
    for (int i = 0; i < block.length; i++){
        ...
        byte unrotated = rotateLeft(block[i], (8 - shift) % 8);
        
        int unadded = (unrotated & 0xFF) - roundNumber;
        block[i] = (byte) (unadded ^ keyBytes[i % keyBytes.length]);
    }
    return block;
}
